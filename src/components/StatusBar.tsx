...
export default function StatusBar() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-3 z-50 flex justify-center px-4">
      <div className="pointer-events-auto flex items-center gap-2.5 rounded-full border border-line bg-panel/85 px-3 py-1.5 shadow-[0_12px_44px_-12px_rgba(0,0,0,0.8)] backdrop-blur sm:gap-3">
        ...
        <div className="flex items-center gap-2 font-mono text-[11px] text-dim">
          ...
          <span className="w-8 text-right text-accent-soft tabular-nums">
            {pct}%
          </span>
        </div>
...
function Spinner() {
  ...
  useEffect(() => {
    if (reduced) return
    const id = window.setInterval(() => setI((v) => (v + 1) % 4), 180)
